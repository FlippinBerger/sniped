...
const GamePage = () => {
  ...
  useQuery('game_fetch', async () => {
    console.log('id: ', id);
    return await apiClient.get(`/game/${id}`);
  },
    {
      onSuccess: (res) => {
      ...
      onError: (err) => {
        console.log(`something happened when fetching game data for id ${id}`, err);
        setErrorState(true);
      },
    });
  ...
  return (
    <div css={pageWrapper}>
      ...
      {!!gameData &&
        <div>
          <h2>Top {gameData.game.name} streams right now</h2>
          <StreamGrid streams={gameData.streams} />
        </div>
      }
    </div>
  )
